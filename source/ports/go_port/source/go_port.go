	...
	case C.METACALL_MAP:
		{
			...
			for i := C.size_t(0); i < size; i++ {
				pairPtr := (*unsafe.Pointer)(unsafe.Pointer(uintptr(unsafe.Pointer(tuples)) + uintptr(i*PtrSizeInBytes)))
				p := reflect.ValueOf(valueToGo(*pairPtr))
			...
			// Note: This preserves all data types but breaks encoding/json serialization.
			// Downstream consumers may experience panics if they attempt to json.Marshal this without conversion.
